...
public class BookingsService {
    ...
    private Bookings documentSnapshotToBookings(DocumentSnapshot document)
    {
        ...
        if(document.exists()){
            bookings = new Bookings(document.getId(),document.getString("userID"),document.getTimestamp("checkInDate").toProto(),document.getTimestamp("checkOutDate").toProto(),document.getDouble("totalPrice"),document.getString("status"),document.getString("paymentStatus"));
        }
        return bookings;
    }
